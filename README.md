...
> **Live demo:** [yashasm18.github.io/specledger](https://yashasm18.github.io/specledger/)<br>
...
| **Use case** | Automated batch processing, nightly jobs | Reviewing the 35.2% of rows that need a human call |
...
Where that path stands: it reads label-value spec sheets well, and wiring-instruction sheets or photo-heavy brochures yield nothing rather than a guess.
...
| `data/samples/sample_datasheet_*.pdf` | Three datasheets used to demonstrate document linking | **Synthetic fixtures, and they say so on their own first line.** Every value in them is one the accompanying catalogue row already states. |
...
The 35.2% that still routes to human review does so for a genuine, disclosed reason. `3M`, `TREX` and `Southwire` used to sit here and now resolve — `3M` because it was already in the store and only the brand lookup couldn't reach it, the others through the curated reference file. `Jam Industrial Supply LLC (JAMIN)` deliberately still does not resolve, for the reason given in [What it achieves](#what-it-achieves): 288 of the 352 remaining rows are that same refusal. Both brand columns are now fully resolved (0 rows under review); everything left is concentrated in `Part_Manuf`.
...
| **Marketing description is left empty** | No source exists for it without a live fetch of the manufacturer's own page, so the field is blank rather than generated. |
